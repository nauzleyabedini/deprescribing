...
.ai-trace-box { background-color: #1e1e1e; color: #4af626; font-family: 'Courier New', Courier, monospace; font-size: 13px; padding: 15px; border-radius: 4px; line-height: 1.8; margin-bottom: 15px; box-shadow: inset 0 0 10px rgba(0,0,0,0.5); }
</style>
...
""", unsafe_allow_html=True)
        ...
        if st.session_state.reconciled:
            # --- DYNAMIC MANUAL AI VISUALIZER (Click-through) ---
            if not st.session_state.animation_played:
                ...
                current_logs = logs[:st.session_state.trace_step + 1]
                log_html = "<br>".join([f"> {l}" for l in current_logs])
                st.markdown(f'<div class="ai-trace-box">{log_html}</div>', unsafe_allow_html=True)
                
                if st.session_state.trace_step < len(logs) - 1:
                    if st.button("Next Step ➔"):
                        st.session_state.trace_step += 1
                        st.rerun()
                else:
                    if st.button("View AI Recommendation ⚡", type="primary"):
    ...
    """, unsafe_allow_html=True)
                        ...
                        if st.button("Re-Apply Auto-Discontinue"):
                            ...
                            st.rerun()
                        addendum_text = "Quetiapine 12.5mg QHS was initiated for ICU delirium. This is being continued at discharge for ***. Recommend PCP assess for ongoing indication and discontinue as soon as possible, as this is not intended for long-term use."
